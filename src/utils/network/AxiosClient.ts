...
//     () =>
//       axios.create({
//         baseURL: 'https://native.onclickindia.com/',
...
const HEAVY_TIMEOUT =120000;   // server ka avg TTFB hi 23s hai, isliye 60s safe buffer
...
const requestWithDiagnostics = async ({ axiosInstance, url, data, config = {}, timeout, method = 'post' }) => {
  const startTime = Date.now();
  const startISO = new Date(startTime).toISOString();

  // rd param ko URL se hi nikal lo, wahi correlation ID hai
  const rdMatch = url.match(/[?&]rd=([^&]+)/);
  const rd = rdMatch ? decodeURIComponent(rdMatch[1]) : 'N/A';

  console.log(`📤 START | rd: ${rd} | ${method.toUpperCase()} ${url} | at ${startISO} | timeout set: ${timeout}ms`);
  ...
  try {
    ...
    console.log(`✅ SUCCESS | rd: ${rd} | duration: ${duration}ms`);

    return { data: response.data, rd, duration, success: true };
    ...
    const report = {
      ...
      timeoutConfiguredMs: timeout,
      firedEarly: duration < timeout - 2000,
      errorCode: error?.code,
      errorMessage: error?.message,
    };

    console.log(`❌ FAILED | rd: ${rd} | duration: ${duration}ms | isTimeout: ${isTimeout}`);
    console.log('📋 DIAGNOSTIC REPORT FOR BACKEND:', JSON.stringify(report, null, 2));
...
const post2 = useCallback(
  async ({ url, data, config = {} }) => {
    const timeout = getTimeoutForUrl(url);
    const result = await requestWithDiagnostics({ axiosInstance, url, data, config, timeout, method: 'post' });
    return result.data;
  },
  [axiosInstance],
);
      ...
      async error => {
        ...
        if (error.response?.status === 401 && !isRefreshing.current) {
          ...
          if (response) {
            ...
            console.warn(
              '⚠️ Refresh token failed. Logging out...',
            );
