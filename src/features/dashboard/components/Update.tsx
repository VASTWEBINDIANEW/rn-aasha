  ...
  useEffect(() => {
    const fetchVersion = async () => {
      try {
        ...
        console.log('====================================45235235235235',id,'ioioio');
        console.log(version);
        setLogoUrl(version.Logo);

        console.log('====================================23523523523',id);
...
const handleUpdate = async () => {
  ...
  try {
    ...
    if (response.isgoogle) {
      ...
      apkUrl = `http://${APP_URLS.baseWebUrl}${APP_URLS.DownloadAPK}`;
      ...
      ReactNativeBlobUtil.config({
        ...
        .then(async (res) => {
          try {
            ...
            // Fallback: browser me APK URL open karo
            Linking.openURL(apkUrl);

            Alert.alert(
              'Installation Failed',
              'Please download and install the APK manually.'
            );
          }
        })
        .catch((downloadError) => {
          ...
          Alert.alert(
            'Update Failed',
            'Could not download APK. Opening download link.'
          );
        });
    }
  } catch (error) {
